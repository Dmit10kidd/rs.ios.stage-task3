...
-(NSString *)findLexicographicallyMinimalForString1:(NSString *)string1 andString2:(NSString *)string2 {
    ...
    while (firstString.length && secondString.length) {
        ...
        if (first[0] < second[0] || first[0] == second[0]) {
            [finalString appendString: [firstString substringToIndex: 1]];
            firstString = [firstString substringFromIndex: 1];
        } else {
            [finalString appendString: [secondString substringToIndex: 1]];
            secondString = [secondString substringFromIndex: 1];
        }
    }
    
    if (firstString.length) {
        [finalString appendString: firstString];
    } else {
        [finalString appendString: secondString];
    }
